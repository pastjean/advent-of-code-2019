...
#[derive(Debug, PartialEq, Clone)]
pub struct Action(Direction, i32);

impl From<&str> for Action {
    fn from(s: &str) -> Self {
        let (dirc, lenc) = s.split_at(1);
        let dir = match dirc {
            "U" => Direction::UP,
            "L" => Direction::LEFT,
            "D" => Direction::DOWN,
...
pub struct PositionDistance(i32, i32, i32);

#[derive(Debug, PartialEq, Copy, Clone)]
pub struct Position(i32, i32);

pub fn enumerate_walk_from(position: &PositionDistance, action: &Action) -> Vec<PositionDistance> {
    (1..=action.1)
        .into_iter()
        .map(|i| match action.0 {
            Direction::LEFT => PositionDistance(position.0 - i, position.1, position.2 + i),
            Direction::RIGHT => PositionDistance(position.0 + i, position.1, position.2 + i),
            Direction::UP => PositionDistance(position.0, position.1 + i, position.2 + i),
            Direction::DOWN => PositionDistance(position.0, position.1 - i, position.2 + i),
        })
        .collect()
}

pub fn enumerate_wire_path(wire: &str) -> Vec<PositionDistance> {
    wire.split(",").map(|a| Action::from(a)).fold(
        vec![PositionDistance(0, 0, 0)],
        |positions, action| {
            let mut new_pos = positions.clone();
            let mut walk = enumerate_walk_from(new_pos.last().unwrap(), &action);
            new_pos.append(&mut walk);
            new_pos
        },
    )
}

pub fn minimun_intersection_distance_between_wires(w1: &str, w2: &str) -> i32 {
    let positions_2: Vec<_> = enumerate_wire_path(w2)
        .iter()
        .map(|p| Position(p.0, p.1))
        .collect();

    enumerate_wire_path(w1)
        .iter()
        .map(|p| Position(p.0, p.1))
        .filter(|p| *p != Position(0, 0))
        .filter(|p| positions_2.contains(p))
        .map(|p| p.0.abs() + p.1.abs())
        .min()
        .unwrap()
}

pub fn minimum_intersection_travel_between_wires(w1: &str, w2: &str) -> i32 {
    let positions_1 = enumerate_wire_path(w1);
    let positions_2 = enumerate_wire_path(w2);

    positions_1
        .iter()
        .filter(|p| *p != &PositionDistance(0, 0, 0))
        .filter_map(|p| {
            let intersection = positions_2
                .iter()
                .filter(|p2| Position(p.0, p.1) == Position(p2.0, p2.1))
                .min_by(|x, y| x.2.cmp(&y.2));

            intersection.map(|i| (p, i, p.2 + i.2))
        })
        .min_by(|x, y| x.2.cmp(&y.2))
        .map(|p| p.2)
        .unwrap()
...
mod tests {
    ...
    fn case1() {
        ...
        assert_eq!(minimun_intersection_distance_between_wires(w1, w2), 159);
    }
    ...
    fn case2() {
        ...
        assert_eq!(minimun_intersection_distance_between_wires(w1, w2), 135);
    }
}
